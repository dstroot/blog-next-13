...
Previously, I wrote about "black swan" technology risks, including technology supply chain risks, in [Risky Business](/posts/2022-03-05-risky-business). As part of any technology risk assessment program we always look closely at third-party risk, especially software that could be "weaponized" against us. Anti-virus software is always something that stands out. This risk is not new: In 2010 [McAfee caused a global IT meltdown](https://www.zdnet.com/article/defective-mcafee-update-causes-worldwide-meltdown-of-xp-pcs/) due to a faulty update. Fun fact: CTO at the time was George Kurtz, current CEO of CrowdStrike.

Anti-virus software generally works by using a signature file that contains patterns of known viruses and if a file is found that matches a pattern, the anti-virus software neutralizes that file. If the signature file could be corrupted, or poisoned in a way that targets system files, it's game over. Or in this case, if a bad kernel driver corrupts your system then it's also game over (Note to Microsoft: "drivers" which cause a BSOD should be disabled _automatically_ on next boot). Interestingly, CrowdStrike only uses a kernel-level driver for Microsoft Windows, not MacOS, or Linux.

Then of course there are the hidden costs in terms of system instability, CPU usage, power usage, heat, and sluggish system performance for all your users due to running anti-malware software. Scanning for viruses consumes, or validating system calls, uses significant resources, slowing down the performance of the system it is meant to protect, and possibly destabilizing it.
...
7. **Performance and Compatibility Issues**: Antivirus software must work seamlessly with various operating systems, applications, and other security tools. Incompatibilities can lead to instability, vulnerabilities, and/or reduced effectiveness.
